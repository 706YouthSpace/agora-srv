...
import { aliCDNSignatureManager } from '../services/url-signature';
import { User } from '../db/user';
import { wxService } from '../services/wexin';
import { oneOffExchangeService } from '../services/one-off-exchange';

const fileServerBaseUri = 'https://cdn.wxa1.706er.com';
...
function signDownloadUrl(fileId: ObjectId) {
    const ts = Date.now() + 1800 * 1000;
    const sigOptions = { filename: `/file/${fileId.toHexString()}`, timestamp: Math.floor(ts / 1000) };
    const signature = aliCDNSignatureManager.signature(sigOptions);

    return `${fileServerBaseUri}${sigOptions.filename}?auth_key=${signature}`;
}
