...
//         onFileBegin: (_name: any, _file:any)=>{    // 文件存储之前对文件进行重命名处理
...
//     return ctx.body = filePath.substr(filePath.lastIndexOf("/")+1,filePath.length); 
// })
...
function getTargetPath(dirName: string,fileName: string) {
    const l1 = dirName.slice(0, 2);
    const l2 = dirName.slice(2, 4);
    const l3 = dirName.slice(4);
    const storeDir = config.storage.sha256Root;
    const targetPath= dirJoin(storeDir,dirJoin(l1, l2, l3),fileName);
    return targetPath ;
}
...
router.get('/uploadFiles/*', async (ctx) => {
    ...
    const originalUrl = ctx.originalUrl;
    const fileName=originalUrl.substr(originalUrl.lastIndexOf("/")+1,originalUrl.length);
    const filePath = getTargetPath(fileName,fileName);
    let file = null;
    try {
